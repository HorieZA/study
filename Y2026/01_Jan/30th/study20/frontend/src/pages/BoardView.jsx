...
const BoardView = () => {
  ...
  const setData = data => {
    setNo(data.no)
    setTitle(data.title)
    setContent(data.cont)
    setName(data.user)
  }
  useEffect(()=>{
    if(!checkAuth()) navigate("/")
    // console.log("cookie", window.atob(cookies.boardInfo) )
    // api.post("/board").then(res=>console.log(res))
    // console.log(params.no)
    // const data = { "no": 1, "title": "샘플을 만들었어요", "cont": "내용을 작성해 주세요.", "user": "이나라" }
    // setData(data)
    api.post("/board").then(res => {
		  console.log(res.data.boardInfo)
		  setData(res.data.boardInfo)
		})
  }, [])
  ...
  if (cookies.nyj) {
    try {
      const decoded = window.atob(cookies.nyj) // base64 decode
      const parsed = JSON.parse(decoded)       // JSON parse
