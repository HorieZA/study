...
const Home = () => {
  ...
  const [cookies, setCookie, removeCookie] = useCookies(['nyj'])
  const searchEvent = e => {
    e.preventDefault()
    //getData(1, search)
    setPage(1)
  ...
  const boardView = v => {
    // setCookie("boardNo", no)
    console.log("누구냐 넌? : ",v.no,"\n누군디 넌? : ",v.title,"\n누구냐 넌또? : ",v.user,"\n누구냐고 넌? : ",v.cont)
    const boardInfo = {
      no: v.no,
      title: v.title,
      user: v.user,
    ...
    // setCookie("boardNo", window.btoa(v.no),"boardTitle", window.btoa(v.title), "boardUser", window.btoa(v.user),"boardCont", window.btoa(v.cont))
    setCookie("boardInfo", window.btoa(encodeURI(JSON.stringify(boardInfo))))
    if(checkAuth()) navigate(`/boardview/${v.no}`)
  }
/*
  const getData = (i, q) => {
  ...
  return (
    <div className="container mt-3">
				...
				<tbody>
          {
            list?.map((v, i) => {
              return (
                <tr className="cursor-pointer" key={i} onClick={()=>boardView(v)}>
                  <td>{v.no}</td>
                  <td>{v.title}</td>
                  <td>{v.user}</td>
                </tr>
