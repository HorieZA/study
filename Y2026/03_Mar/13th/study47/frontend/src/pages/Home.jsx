...
const Home = () => {
  ...
  // const eventSubmit = e => {
  //   e.preventDefault()
  //   api.get("/webhook-test/app", {params: item})
  //   .then( res => {
  //     // console.log(res)
    ...
    <div className="container mt-3">
      ...
      <form onSubmit={eventSubmit}>
        <div className="mb-3">
          ...
          <input type="email" className="form-control" name="email" id="email" placeholder="name@example.com" value={item.email} onChange={setChange} required/>
        </div>
        <div className="mb-3">
      ...
      <div className="list-group mt-3">
        {
          list.map((v, i) => <button key={i} type="button" className="list-group-item list-group-item-action">{v.content}</button>)
        }
      </div>
		</div>
  )
