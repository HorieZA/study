...
const Create = () => {
  ...
  const onSubmit = e => {
    e.preventDefault()
    const data = { title, content, accept:2 }
    console.log(data)
  }
      ...
      <form onSubmit={onSubmit}>
        <div className="mb-3 mt-3">
          ...
          <input type="text" className="form-control" id="title" placeholder="Enter title"
            name="title" required autoComplete="off" onChange={e => setTitle(e.target.value)} />
        </div>
        <div className="mb-3">
          <label htmlFor="content" className="form-label">Content:</label>
          <input type="text" className="form-control" id="content" placeholder="Enter content"
            name="content" autoComplete="off" onChange={e => setContent(e.target.value)} />
        </div>
