...
# 커스텀 부분 => "<h1 style='text-align: center;'>수집 목록</h1>"
st.markdown("<h1 style='text-align: center;'>수집 목록</h1>", unsafe_allow_html=True)

# HTML 태그이며, select에서 사용되는 함수 이며 value용
links = [
...
# HTML 태그이며, select에서 사용되는 함수 이며 보여주기용
...
def main():
  try:
    ...
    gnrCode = url.split("gnrCode=")[1].split("&")[0]
    ...
    if res.status_code == 200:
      ...
      if len(arr) > 0:
        sql1 = f"DELETE FROM edu.`melon` WHERE `code` = '{gnrCode}'"
        sql2 = f"""
        ...
        values = [(gnrCode, row["id"], row["img"], row["title"], row["album"], row["cnt"]) for row in arr]
